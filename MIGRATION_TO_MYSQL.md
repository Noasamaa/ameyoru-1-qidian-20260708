...
或者把 `drizzle/*.sql` 按文件名顺序直接 `mysql ... < ` 灌进去也行,只用一次。

跑完后用 `SHOW TABLES;` 确认 9 张表都建好:`account, customer, customer_balance_txn, customer_balance_txn_player, order, player_invite, session, user, verification`。
