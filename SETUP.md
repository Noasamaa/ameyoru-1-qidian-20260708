...
| **PLAYER** (陪玩) | 接单赚钱 | 快速接单/报单(单价锁定)、看自己的订单/排行/打款明细 |
...
陪玩主页有"开始接单"卡片:
1. 填客户名 → 点击"开始接单"→ 实时计时开始
2. 服务结束 → 点击"结束接单"→ 按实际时长自动算金额,订单直接标记为已完成
3. 管理端"待结算"里出现该订单 → 打款

单价由老板设定,陪玩不可修改(UI 锁定 + 服务端强制)。
...
5. `tutu` 登录 → 主页"开始接单",填客户名 → 结束接单 → 订单自动完成
